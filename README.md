...
### О решении
Настроены 2 docker compose сервиса:\
db - PostgreSQL\
api - FastAPI

Перед запуском можно добавить файл `.env` с логином и паролем к базе данных и портом приложения

Запуск проекта возможен командой:\
`docker compose up --build` (режим разработчика - hot reload приложения при изменении файлов)\
или\
`docker compose -f docker-compose.yaml up --build` (режим "продакшена")

Требуемые в задаче эдпоинты по умолчанию доступны по адресам:\
`localhost:8000/api/comments`\
`localhost:8000/api/general`

Дополнительно настроена небольшая веб-страница для наглядной демонстрации работы системы.\
